...
export const Profiles = {
  ...
  // ─── Check & update streak ───
  // Returns { streak, isNew, isMilestone } where isMilestone = true on 3,7,14,30,50,100 days
  checkStreak(id) {
    const MILESTONES = [3, 7, 14, 30, 50, 100];
    const profiles = this.all();
    const p = profiles.find(x => x.id === id);
    if (!p) return { streak: 0, isNew: false, isMilestone: false };
    const today = new Date().toDateString();
    if (p.lastDate === today) return { streak: p.streak || 0, isNew: false, isMilestone: false };
    const yesterday = new Date(Date.now() - 86400000).toDateString();
    const prevStreak = p.streak || 0;
    p.streak = (p.lastDate === yesterday) ? prevStreak + 1 : 1;
    if (p.streak > (p.bestStreak || 0)) p.bestStreak = p.streak;
    p.lastDate = today;
    p.completedToday = [];
    save(KEY_PROFILES, profiles);
    const isNew = p.streak > prevStreak;
    const isMilestone = isNew && MILESTONES.includes(p.streak);
    return { streak: p.streak, isNew, isMilestone };
  },

  // ─── Record completed task ───
  completeTask(id, world, stepIndex, meta = {}) {
    const profiles = this.all();
    const p = profiles.find(x => x.id === id);
    if (!p) return null;
    ...
    // ─── History (last 50 completed steps) ───
  ...
  getHistory(id, limit = 50) {
    ...
    return (p?.history || []).slice(0, limit);
  },
